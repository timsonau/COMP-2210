...
public class Boggle implements WordSearchGame {
   ...
   public SortedSet<String> getAllScorableWords(int minimumWordLength){
      ...
      for(String word : lexicon) {
         ...
         if(path.size() >= minimumWordLength) {
   ...
   */  
   public int getScoreForWords(SortedSet<String> words, int minimumWordLength){
      ...
      for(String word : words) {
         if(lexicon.contains(word)) {
            List<Integer> path = isOnBoard(word);
            if(path.size() >= minimumWordLength) {
               score += (1+ (path.size() - minimumWordLength));
            }
         }
      }
      return score;
   ...
   public List<Integer> isOnBoard(String wordToCheck){
      ...
      Position[] positions = letterPositions(wordToCheck, 0);
      
      if(positions.length == 0 || wordToCheck.length() > (numColumns * numRows)) return path;

      
      for(Position p : positions) {
         wordBeingBuilt = wordToCheck.substring(0, 1);
         visit(p);
    ...
    */
   private Position[] letterPositions(String word, int index) {
      if(index >= word.length()) {
         throw new IllegalArgumentException("invalid index for a given word");
      }
